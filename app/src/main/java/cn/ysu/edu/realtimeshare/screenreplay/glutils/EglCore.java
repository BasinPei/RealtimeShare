...
public final class EglCore {
     ...
     */
    public EglCore(EGLContext sharedContext, final int flags) {
        ...
        if ((flags & FLAG_TRY_GLES3) != 0) {
            ...
            final EGLConfig config = getConfig(flags, 3);
            if (config != null) {
                final int[] attrib3_list = {
                        EGL14.EGL_CONTEXT_CLIENT_VERSION, 3,
                        EGL14.EGL_NONE
                };
                final EGLContext context = EGL14.eglCreateContext(mEGLDisplay, config, sharedContext,
                        attrib3_list, 0);
        ...
        if (mEGLContext == EGL14.EGL_NO_CONTEXT) {  // GLES 2 only, or GLES 3 attempt failed
            ...
            final EGLConfig config = getConfig(flags, 2);
            if (config == null) {
    ...
    public EGLSurface createWindowSurface(final Object surface) {
        ...
        final EGLSurface eglSurface = EGL14.eglCreateWindowSurface(mEGLDisplay, mEGLConfig, surface,
                surfaceAttribs, 0);
    ...
    public EGLSurface createOffscreenSurface(final int width, final int height) {
        ...
        final EGLSurface eglSurface = EGL14.eglCreatePbufferSurface(mEGLDisplay, mEGLConfig,
                surfaceAttribs, 0);
