...
package cn.ysu.edu.realtimeshare.librtsp.audio;
...
import cn.ysu.edu.realtimeshare.librtsp.rtp.AACADTSPacketizer;
import cn.ysu.edu.realtimeshare.librtsp.rtp.AACLATMPacketizer;
 ...
 * You should use a {@link Session} instantiated with librtsp.SessionBuilder} instead of using this class directly.
 * Call {@link #setDestinationAddress(InetAddress)}, {@link #setDestinationPorts(int)} and #setAudioQuality(librtsp.audio.AudioQuality)}
 * to configure the stream. You can then call {@link #start()} to start the RTP stream.
 * Call {@link #stop()} to stop the stream.
 */
public class AACStream extends cn.ysu.edu.realtimeshare.librtsp.audio.AudioStream {
	...
	protected void encodeWithMediaRecorder() throws IOException {
		testADTS();
		((cn.ysu.edu.realtimeshare.librtsp.rtp.AACADTSPacketizer)mPacketizer).setSamplingRate(mQuality.samplingRate);
	...
	protected void encodeWithMediaCodec() throws IOException {
		...
		final int bufferSize = AudioRecord.getMinBufferSize(mQuality.samplingRate, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT)*3;

		((cn.ysu.edu.realtimeshare.librtsp.rtp.AACLATMPacketizer)mPacketizer).setSamplingRate(mQuality.samplingRate);
